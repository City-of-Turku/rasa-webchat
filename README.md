...
| `newIdOnReset` | `false`            | If `true`, when conversation history is deleted, a new session id will be issues (and a new socket connection established). If `false` existing session and session id are used. |
| `resetPayload`          | `null`             | Payload sent to Rasa when user resets the conversation from the reset button. If not set, `initPayload` is sent instead. |
...
| `onWidgetEvent`        | `{}`             | call custom code on a specific widget event ( `onChatOpen`, `onChatClose`, `onChatHidden`, and `onChatReset`, are available for now ), add a function to the desired object property in the props to have it react to the event. |
